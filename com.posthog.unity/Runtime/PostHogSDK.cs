...
public class PostHogSDK : MonoBehaviour
{
    ...
    )
    {
        ...
        return _instance.IdentifyInternalAsync(distinctId, userProperties, userPropertiesSetOnce);
    }
    ...
    async Task ResetInternalAsync()
    {
        ...
        _featureFlagManager.ResetGroupPropertiesForFlags(_storage, reloadFeatureFlags: false, this);
    ...
    static bool EnsureInitialized()
    {
        if (!_isInitialized)
        {
            PostHogLogger.Warning("PostHog SDK not initialized. Call PostHogSDK.Setup() first.");
            return false;
        }
        return true;
    }
