...
class EventQueue
{
    ...
    public EventQueue(PostHogConfig config, IStorageProvider storage, NetworkClient networkClient)
    {
        _config = config;
        _storage = storage;
    ...
    public void Enqueue(PostHogEvent evt)
    {
        lock (_lock)
        {
            ...
            if (eventIds.Count >= _config.MaxQueueSize)
            {
                ...
                PostHogLogger.Warning($"Queue full ({_config.MaxQueueSize}), dropped oldest event");
            }
            ...
            while (true)
            {
                ...
                else
                {
                    ...
                    else
                    {
                        // Pause for retry
                        _retryCount++;
                        int delay = Math.Min(_retryCount * RetryDelaySeconds, MaxRetryDelaySeconds);
    ...
    PostHogEvent DeserializeEvent(string json)
    {
        try
        {
            ...
            var evt = new PostHogEvent
            {
                ...
                Event = dict.TryGetValue("event", out var eventName) ? eventName?.ToString() : null,
